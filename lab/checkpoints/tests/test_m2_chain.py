...
def test_merge_has_three_inputs_and_feeds_assemble(wf):
    n = node(wf, "Merge verdicts")
    assert n["type"] == "n8n-nodes-base.merge"
    assert n["parameters"]["numberInputs"] == 3
    assert targets(wf, "Merge verdicts") == [("Assemble verdicts", 0)]


def test_assemble_builds_the_gather_document(wf):
    code = node(wf, "Assemble verdicts")["parameters"]["jsCode"]
    for needle in ("indicatorVerdicts", "wazuhEvents", "caseId", "hash"):
...
def test_render_prepends_indicator_table(wf):
    value = assignment(wf, "Render verdict", "verdict")
