...
2. **Wazuh by host.** `wazuh_events_for_host`, description:
   ...
   Same URL and auth, body:

   ```json
   { "size": 20, "_source": ["timestamp","rule.id","rule.level","rule.description","data.srcip","data.url","data.dstuser","agent.name"], "query": { "match": { "agent.name": "{{ $fromAI('host', 'Wazuh agent name of the host', 'string') }}" } }, "sort": [ { "timestamp": "desc" } ] }
   ```
...
The workflow now runs end to end, with the agent choosing its own lookups. This exercise is the functional test: given a real case, when nobody touches a keyboard, then the trace shows what the agent chose and the verdict carries its determination.
