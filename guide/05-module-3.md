...
> **TBC.** What happens exactly when the agent hits the iteration cap.
...
```
GET http://thehive:9000/api/v1/case/{caseId}
...
```
POST http://thehive:9000/api/v1/query
Authorization: Bearer <your TheHive API key>
...
The HTTP Request nodes you built in Module 2 (each one a lookup source like Wazuh or the auth log lookup) are now tools the agent can invoke. You no longer control when they run. The agent reads the case, decides which tools will help, and calls them in the order it chooses.
...
The agent needs two additions to its system prompt:

1. **Give it permission.** "You may use these tools to investigate the case and gather evidence." This signals that tool calling is not a fallback; it is the intended mode.
...
These two additions guide the agent without removing its reasoning.
